    ...
    const char *embed_input_file = "https://www.phenix.bnl.gov/WWW/publish/phnxbld/sPHENIX/files/sPHENIX_G4Hits_sHijing_9-11fm_00000_00010.root")
{
  ...
  else
  {
    ...
    if (usegun)
    {
      ...
      pgen->set_eta_range(-.01, .0);
      pgen->set_mom_range(29.9, 30.1);
      pgen->set_phi_range(0., 2.*TMath::Pi());
...
//    in1->AddListFile(embed_input_file);  // Recommended: if one use a text list of many input files
  ...
  std::string jetoutputFile = std::string(outputFile) + std::string("_pionGun+jets.root");
  MyJetAnalysis *myJetAnalysis = new MyJetAnalysis("AntiKt_Track_r03","AntiKt_Truth_r03",jetoutputFile.data());
